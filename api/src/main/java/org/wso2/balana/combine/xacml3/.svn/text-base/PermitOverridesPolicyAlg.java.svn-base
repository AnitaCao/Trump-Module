...
/**
 * This is the new (XACML 3.0) is the standard Permit Overrides policy combining
 * algorithm. It allows a single evaluation of Permit to take precedence over
 * any number of deny, not applicable or indeterminate results. Note that since
	...
	public AbstractResult combine(EvaluationCtx context, List parameters,
		...
		if (atLeastOneErrorP && (atLeastOneErrorD || atLeastOneDeny)) {
			...
			return ResultFactory.getFactory().getResult(
					AbstractResult.DECISION_INDETERMINATE_DENY_OR_PERMIT,
					firstIndeterminateResultP.getStatus(), context);
		}

		if (atLeastOneErrorP) {
			return ResultFactory.getFactory().getResult(
					AbstractResult.DECISION_INDETERMINATE_PERMIT,
					firstIndeterminateResultP.getStatus(), context);
		}
