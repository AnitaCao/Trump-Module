...
/**
 * This is the standard Deny unless permit policy combining algorithm. This
 * algorithm is intended for those cases where a permit decision should have
 * priority over a deny decision, and an “Indeterminate” or
 * “NotApplicable” must never be the result. It is particularly useful at
 * the top level in a policy structure to ensure that a PDP will always return a
 * definite “Permit” or “Deny” result.
 */
public class DenyUnlessPermitPolicyAlg extends PolicyCombiningAlgorithm {
	...
	public AbstractResult combine(EvaluationCtx context, List parameters,
		...
		for (Object policyElement : policyElements) {
			AbstractPolicy policy = ((PolicyCombinerElement) (policyElement))
					.getPolicy();
		...
		return ResultFactory.getFactory().getResult(
				AbstractResult.DECISION_DENY, denyObligations, denyAdvices,
				context);
	}
}
