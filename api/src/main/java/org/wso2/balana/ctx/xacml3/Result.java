...
public class Result extends AbstractResult {
	...
	public Result(int decision, Status status,
			...
			EvaluationCtx evaluationCtx) throws IllegalArgumentException {
	...
	public Result(int decision, Status status,
			List<ObligationResult> obligationResults, List<Advice> advices,
			Set<PolicyReference> policyReferences, Set<Attributes> attributes)
			throws IllegalArgumentException {
		super(decision, status, obligationResults, advices);
			...
			if (name.equals("Decision")) {
				...
				if (status == null) {
					...
					throw new ParsingException(
							"More than one StatusType defined");
				}
				...
				if (obligations == null) {
					...
					throw new ParsingException(
							"More than one ObligationsType defined");
				}
				...
				if (advices == null) {
					...
					throw new ParsingException(
							"More than one AssociatedAdviceType defined");
				}
				...
				if (policyReferences == null) {
					...
					throw new ParsingException(
							"More than one PolicyIdentifierListType defined");
				}
	 ...
	 * 
	 * @param root
	 *            the DOM root of the ObligationsType XML type
	 * @return a <code>List</code> of <code>ObligationResult</code>
	 * @throws ParsingException
	 *             if any issues in parsing DOM
	 */
	 ...
	 * 
	 * @param root
	 *            the DOM root of the AssociatedAdviceType XML type
	 ...
	 * 
	 * @param root
	 *            the DOM root of the PolicyIdentifierListType XML type
	...
	public void encode(OutputStream output, Indenter indenter) {
		...
		if (decision == 4 || decision == 5 || decision == 6) {
			// if this is extended indeterminate values, we just return the
			// "Indeterminate"
			out.println(indentNext + "<Decision>" + DECISIONS[2]
					+ "</Decision>");
		} else {
