...
public class AttributeAssignment extends AttributeValue {
	...
	/**
	 * category of the AttributeAssignment element whether it is subject, action
	 * and etc
	 */
	...
	/**
	 * Constructor that creates a new <code>AttributeAssignment</code> based on
	 * the given elements.
	 * 
	 * @param attributeId
	 *            attribute id of the AttributeAssignment element
	 ...
	 *            attributes datatype
	 * @param category
	 *            category of the AttributeAssignment element whether it is
	 ...
	 *            issuer of the AttributeAssignment
	 */
	public AttributeAssignment(URI attributeId, URI dataType, URI category,
			String content, String issuer) {
		super(dataType);
	...
	/**
	 * 
	 * <code>AttributeAssignment</code> based on its DOM node.
	 * 
	 * @param root
	...
	public static AttributeAssignment getInstance(Node root)
		...
		if (!root.getNodeName().equals("AttributeAssignment")) {
			throw new ParsingException(
					"AttributeAssignment object cannot be created "
		...
		try {
			type = new URI(nodeAttributes.getNamedItem("DataType")
					.getNodeValue());
		} catch (Exception e) {
			throw new ParsingException("Error parsing required AttributeId in "
					+ "AttributeAssignmentType", e);
		...
		return new AttributeAssignment(attributeId, type, category, content,
				issuer);
	}
	...
	/**
	 * Encodes this <code>AttributeAssignment</code> into its XML form and
	 * writes this out to the provided
