...
public class FileBasedPolicyFinderModule extends PolicyFinderModule {
	...
	public void loadPoliciesDir(File dir)
	{
		...
		for(File f : dir.listFiles())
		{
			...
			else
			{
				if(f.getName().endsWith(".xml"))
					loadPolicy(f.getAbsolutePath(),finder);
			}
		}
	}
	...
	public void loadPolicies() {
		...
		{
			File dir = new File(policyLocation);
			loadPoliciesDir(dir);
		}
		...
		for (String policyLocation : policyLocations) {
			...
			if (file.isDirectory()) {
				...
				for (String policyFile : files) {
					loadPolicy(policyLocation + File.separator + policyFile,
							finder);
				}
			} else {
