...
public class SubStringFunction extends FunctionBase {
	 ...
	 */
	public static final String NAME_STRING_SUB_STRING = FUNCTION_NS_3
			+ "string-substring";

	/**
	 * Standard identifier for the any uri sub string function.
	 */
	public static final String NAME_ANY_URI_SUB_STRING = FUNCTION_NS_3
			+ "anyURI-substring";
	...
	public SubStringFunction(String functionName) {
		super(functionName, getId(functionName), getArgumentType(functionName),
				false, 3, StringAttribute.identifier, false);
	}
	...
	public EvaluationResult evaluate(List<Evaluatable> inputs,
		...
		String processedString = argValues[0].encode().substring(
				Integer.parseInt(argValues[1].encode()),
