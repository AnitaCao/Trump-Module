...
/**
 * Represents the TargetType XML type in XACML 3.0. This extends the
 * AbstractTarget. This also stores several other XML type: AnyOf
 */
public class Target extends AbstractTarget {
	 ...
	 * components.
	 * 
	 * @param anyOfSelections
	 *            List of <code>AnyOfSelection</code> objects that representing
	 *            the AnyOf sections of this target
	 */
	...
	public static Target getInstance(Node root, PolicyMetaData metaData)
		...
		for (int i = 0; i < children.getLength(); i++) {
			...
			if ("AnyOf".equals(child.getNodeName())) {
				anyOfSelections
						.add(AnyOfSelection.getInstance(child, metaData));
			}
		}
